...
namespace Exercice_Marché
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            // On filtre les produits "Pastèques" version avec LINQ
            /*var pastèques = products.Where(p => p.Name.ToLower() == "pastèques");

            if (pastèques.Any())
            {
                // On prend le produit avec la quantité max
                var maxPastèque = pastèques.OrderByDescending(p => p.Quantity).First();

                // On affiche le message avec string interpolation
                Console.WriteLine($"C'est {maxPastèque.Provider} qui a le plus de pastèques (stand {maxPastèque.Location}, {maxPastèque.Quantity} {maxPastèque.Unit})");
            }
            else
            {
                Console.WriteLine("Il n'y a pas de pastèques disponibles.");
            }*/

            Product maxPastèque = null;
            int maxQuantity = -1;
